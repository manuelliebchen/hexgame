...
vec2 Field::getVectorPosition(Tile* tile) const
{
    ...
    int      x             = tile_position % size[0];
    int      y             = tile_position / size[0];
    return vec2((x + (y % 2 == 1 ? 0.25f : -0.25) + 0.5f) * three_sqrt_half,
                (y + 0.5f) * 0.75f);
}
...
vec2 Field::getDrawingPosition(Tile* pos) const
{
    return getVectorPosition(pos) +
           (pos->getHeight() > 0 ? vec2(0, pos->getHeight() * TILE_STEP_HEIGHT)
                                 : vec2());
}
                                   ...
                                   Tile* destination_tile) const
{
    ...
    //The Open Nodes
    auto cmp = [&costs](Tile* pos1, Tile* pos2) -> bool {
        return costs[pos1] > costs[pos2];
    };
    ...
    while (!queue.empty())
    {
        ...
        for (Tile* neighbor : getSurounding(top))
        {
            ...
            if (!neighbor->isWalkable() ||
                height_diference >= PATH_HEIGHT_DISCREPANCY)
            {
                continue;
            }
            ...
            if (way_length.find(neighbor) != way_length.end() &&
                way >= way_length[neighbor])
            {
                continue;
            }
...
std::vector<Tile*> Field::findSurounding(Tile* start_tile, int n) const
{
    ...
    //The Open Nodes
    auto cmp = [&way_length](Tile* pos1, Tile* pos2) -> bool {
        return way_length[pos1] > way_length[pos2];
    };
    std::priority_queue<Tile*, std::vector<Tile*>, decltype(cmp)> queue(cmp);
    std::vector<Tile*>                                            open;
